...
class TopicListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # url = reverse('boards:topic_posts', kwargs={'pk': self.board.pk, 'topic_pk': self.pk})
...
class PostListView(ListView):
    ...
    def get_queryset(self):
        ...
        querytset = self.topic.posts.order_by('created_at')
        return querytset
...
def reply_topic(request, pk, topic_pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            post.save()
            url = reverse('boards:topic_posts',kwargs={'pk':pk, 'topic_pk':topic_pk})
            post_url = '{url}?page={page}'.format(url=url, page=topic.get_page_count())
            print(post.pk)
